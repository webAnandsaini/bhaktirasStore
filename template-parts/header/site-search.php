...
<div class="site-search-form flex-1 max-w-[735px] mx-auto px-2 lg:px-6">
    <form role="search" method="get" class="relative flex items-center w-full" action="<?php echo esc_url(home_url('/')); ?>" aria-label="<?php esc_attr_e('Site Search', 'dharmgyan'); ?>" onsubmit="var val = this.querySelector('input[name=s]').value.trim(); if (!val) { this.querySelector('input[name=s]').focus(); return false; }">
        ...
        <button 
            type="submit" 
            class="absolute right-0 top-0 bottom-0 px-3.5 flex items-center justify-center text-[#737373] hover:text-[#CC5600] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#CC5600] cursor-pointer"
