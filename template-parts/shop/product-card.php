...
<div class="product-card group flex flex-col justify-between bg-white border border-[#EAE3DC] hover:border-[#CC5600] rounded-[6px] p-2.5 sm:p-3 shadow-2xs hover:shadow-md transition-all duration-300 relative" data-product-id="<?php echo esc_attr($product_id); ?>">
    <div>
        <!-- Product Thumbnail with overlaid actions -->
        <div class="relative aspect-square w-full rounded-[4px] overflow-hidden bg-[#F9F9F9]">
            <a href="<?php echo esc_url($product_link); ?>" class="block w-full h-full" tabindex="-1">
                <img
            ...
            <!-- Sale Badge (top-left) -->
            <?php if ($is_on_sale): ?>
                <span class="absolute top-2.5 left-2.5 bg-[#CC5600] text-white text-[11px] font-bold px-2 py-0.5 rounded-[3px] shadow-sm z-10 select-none">
            ...
            <div class="product-card-wishlist absolute top-2.5 right-2.5 z-20">
                <button
                    type="button"
                    class="dharmgyan-wishlist-toggle-btn w-8 h-8 flex items-center justify-center bg-white/95 hover:bg-white rounded-full shadow-sm hover:scale-110 transition-all duration-150 cursor-pointer border border-[#EAE3DC] hover:border-[#CC5600] text-[#555555] hover:text-[#CC5600] <?php echo $is_in_wishlist ? 'is-in-wishlist' : ''; ?>"
                    aria-label="<?php esc_attr_e('Toggle Wishlist', 'dharmgyan'); ?>"
                    data-product-id="<?php echo esc_attr($product_id); ?>"
                >
                    <svg class="heart-icon w-4 h-4 transition-all duration-200" viewBox="0 0 24 24" fill="<?php echo $is_in_wishlist ? '#CC5600' : 'none'; ?>" stroke="<?php echo $is_in_wishlist ? '#CC5600' : 'currentColor'; ?>" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-2.5 z-10 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out">
                ...
                <a
                    ...
                    class="add_to_cart_button ajax_add_to_cart w-full inline-flex items-center justify-center gap-2 bg-[#CC5600] hover:bg-[#B34B00] text-white text-xs md:text-[13px] font-semibold py-2.5 px-3 rounded-[4px] shadow-md transition-all duration-200"
                    aria-label="<?php echo esc_attr($cart_desc); ?>"
                >
                    <svg class="cart-icon w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <?php if ($rating_count > 0): ?>
                <div class="flex items-center gap-1.5 mt-1">
                    <div class="flex items-center gap-0.5" aria-label="<?php echo esc_attr(sprintf(__('Rated %s out of 5', 'dharmgyan'), number_format($average, 1))); ?>" role="img">
                        <?php for ($i = 1; $i <= 5; $i++):
                            if ($i <= floor($average)): ?>
                                <svg class="w-[11px] h-[11px] text-[#FFB23D]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                            <?php elseif (($i - 0.5) <= $average): ?>
                                <svg class="w-[11px] h-[11px] text-[#FFB23D]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2v15.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                            <?php else: ?>
                                <svg class="w-[11px] h-[11px] text-[#E5E5E5]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                            <?php endif;
                        endfor; ?>
                    </div>
                    <span class="text-[#717171] text-[11px] font-body">(<?php echo esc_html($rating_count); ?>)</span>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="px-1 flex items-baseline gap-1 mt-1.5">
